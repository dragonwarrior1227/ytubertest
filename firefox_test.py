...
try:
    ...
    # print(subprocess.Popen("""ln -s /usr/local/firefox/firefox /usr/bin/firefox""",shell=True,stdout=subprocess.PIPE).communicate()[0])
    print(subprocess.Popen("/usr/local/firefox/firefox --help",shell=True,stdout=subprocess.PIPE).communicate()[0])
    options = Options()
    options.binary_location =r'/usr/local/firefox/firefox'
