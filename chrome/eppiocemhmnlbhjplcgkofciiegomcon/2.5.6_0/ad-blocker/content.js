...
var PosdAdsManager = /*#__PURE__*/function () {
  function PosdAdsManager(id, Config, IOManager, AdBlockerAgent) {
    ...
    this.SkinAds = null;
    this.HTML5Ads = null;
    this.VideoAds = null; // global black list
...
var PosdContentIOManager = /*#__PURE__*/function () {
  ...
  _createClass(PosdContentIOManager, [{
    ...
    value: function SendFBInStreamData(id, inStreamData) {
      var mes = PosdMessage.EmptyMessage;
      var strJSON = JSON.stringify(inStreamData.content);
      inStreamData.encodedText = PosdBase64.encode(strJSON);
  ...
  _createClass(PosdFrameContent, [{
    ...
    value: function InjectJs() {
      ...
      script.innerHTML = "window.bisData = " + JSON.stringify(bisData) + ";" + PosdWindowTargetUrlDetector.GetWindowEmbeddedTargetUrlDataList.toString() + "(" + function () {
        ...
        }, false); //TODO: move to PosdWindowTargetUrlDetector

        function GetWindowClickTargetUrl() {
          if (window.bisData) {
            var clickElementsSelector = window.bisData.html5TargetUrlDetectionConfig.TARGET_URL_CLICK_ELEMENTS_SELECTOR; //to config

            var clickElements = document.querySelectorAll(clickElementsSelector);

            if (clickElements.length) {
              //resolve by closure
              window.bisData.origWindowOpen = window.open; //resolve by closure
  ...
  _createClass(PosdSkinAds, [{
    ...
    value: function GetSkinAdFromBodyBackground() {
      ...
      if (candidate.source.length && candidate.targetUrl.length) {
        this.hideElement(document); // if (this.AdBlockerAgent.IsEnabled()) {
...
var PosdAdDetector = /*#__PURE__*/function () {
  ...
  _createClass(PosdAdDetector, [{
    ...
    value: function IsActualByKeyWordInUrl() {
...
var PosdFacebookAds = /*#__PURE__*/function (_PosdAdsCandidates) {
  ...
  _createClass(PosdFacebookAds, [{
    ...
    value: function ActivateDetector(activator) {
      ...
      var versionDetected = false; // this.fbContent = document.getElementsByClassName(this.fbConfig.fbContent)[0];
      // if (this.fbContent) {
...
var PosdFacebookCommon = /*#__PURE__*/function () {
  ...
  _createClass(PosdFacebookCommon, null, [{
    ...
    value: function GetVisibleSpansText(a, selector) {
      ...
      if (a.hasAttribute('bis_sponsor_checked') && a.getAttribute('bis_sponsor_checked') == limit) {
        return '';
      }

      var elms = a.querySelectorAll(selector);
      var fullText = '';
      var currentTry = a.getAttribute('bis_sponsor_checked') || 0;
      ...
      if (listAllies.length && !listAllies.includes(fullText)) {
    ...
    value: function checkLettersOfAlly(text, listAllies) {
      ...
      while (!foundAlly && currentTry <= maxTries) {
        ...
        var check = listAllies[allyIndex].indexOf(splitText[textIndex]);
        ...
        if (check > -1 && lastIndexText !== textIndex) {
          textIndex += 1;
        } else if (check > -1 && lastIndexText === textIndex && text.length === listAllies[allyIndex].length) {
          foundAlly = true;
          ally = listAllies[allyIndex];
        } else {
          currentTry += 1;
          textIndex = 0;
        }
    ...
    value: function GetMinImgsSpacePercentag(fbConfig, adPlacementType, designVersion) {
      if (adPlacementType === PosdConst.FACEBOOK_AD_PLACEMENT_TYPE_FEED) {
        // if (designVersion === PosdConst.FACEBOOK_DESIGN_VERSION_OLD) {
        //     return fbConfig.feedMinImgsSpacePercentage;
        // } else {
        return fbConfig.feedNewMinImgsSpacePercentage; // }
      } else if (adPlacementType === PosdConst.FACEBOOK_AD_PLACEMENT_TYPE_MARKETPLACE) {
        // if (designVersion === PosdConst.FACEBOOK_DESIGN_VERSION_OLD) {
        //     return fbConfig.marketplaceAds.minImgsSpacePercentage;
        // } else {
        ...
        //   if (designVersion === PosdConst.FACEBOOK_DESIGN_VERSION_OLD) {
        //       return fbConfig.rightColumnAds.minImgsSpacePercentage;
        //  } else {
...
var PosdFacebookFeed = /*#__PURE__*/function (_PosdAdDetector) {
  ...
  _createClass(PosdFacebookFeed, [{
    ...
    value: function ActualizeDetector() {
      var result = false;
      var feed = null; //   if (this.designVersion === PosdConst.FACEBOOK_DESIGN_VERSION_OLD) {
      //      feed = document.getElementById(this.fbConfig.feed);
      //   } else {

      feed = document.querySelector(this.fbConfig.feedNew); //   }
    ...
    value: function Detector() {
      if (this.designVersion) {
        var fbFeed = null; // if (this.designVersion === PosdConst.FACEBOOK_DESIGN_VERSION_OLD) {
    ...
    value: function CheckIfAd(post) {
      //   if (this.designVersion === PosdConst.FACEBOOK_DESIGN_VERSION_OLD) {
      //       return this.fbConfig.possibleSponsoredTextQueries.some((query) => {
      //           var result = post.querySelectorAll(query);
      //           return [...result].some((t) => {
      //               var isAd = false;
      //               var visibleText = '';
      //               var rawVisibleText = PosdFacebookCommon.GetVisibleText(t);
      //               if (typeof rawVisibleText === "string") {
      //                  visibleText = rawVisibleText;
      //               } else if (rawVisibleText.join) {
      //                  visibleText = rawVisibleText.join('');
      //               }
      //               if (this.fbConfig.sponsoredTexts.some(sponsoredText => visibleText.indexOf(sponsoredText) !== -1)) {
      //                   t.setAttribute('bis_label', 'fb_feed_ad');
      //                   isAd = true;
      //               }
      //               if (!isAd) {
      //                  var deepText = PosdFacebookCommon.GetVisibleSpansText(t, this.fbConfig.visibleSpansText);                     
      //                  if (deepText !== visibleText) {
      //                     if (this.fbConfig.sponsoredTexts.some(sponsoredText => deepText.indexOf(sponsoredText) !== -1)) {
      //                        t.setAttribute('bis_label', 'fb_feed_ad');
      //                        isAd = true;
      //                     }
      //                  }
      ...
      for (var i = 0; i < links.length && !result; i++) {
        ...
        var attrText = links[i].getAttribute(this.options.attributeWithSponsorAlly);
        ...
        if (typeof text === "string" && text.length && this.isTagSponsored(argumentsToCheck)) {
          ...
          var deepText = PosdFacebookCommon.GetVisibleSpansText(links[i], this.fbConfig.visibleSpansText, this.fbConfig.sponsoredTexts);
...
var PosdFacebookMarketplace = /*#__PURE__*/function (_PosdAdDetector) {
  ...
  _createClass(PosdFacebookMarketplace, [{
    ...
    value: function CheckIfAd(candidate) {
      ...
      if (candidate) {
        if (this.designVersion === PosdConst.FACEBOOK_DESIGN_VERSION_NEW) {
          var sponsorBlocks = Array.from(candidate.querySelectorAll(this.fbConfig.marketplaceAdsNew.sponsorBlocks));

          if (sponsorBlocks && sponsorBlocks.length) {
            isAd = sponsorBlocks.some(function (sponsorBlock) {
              var deepText = PosdFacebookCommon.GetVisibleSpansText(sponsorBlock, _this4.fbConfig.visibleSpansText);

              if (_this4.fbConfig.sponsoredTexts.some(function (sponsoredText) {
...
var PosdFacebookRightColumn = /*#__PURE__*/function (_PosdAdDetector) {
  ...
  _createClass(PosdFacebookRightColumn, [{
    key: "ActualizeDetector",
    value: function ActualizeDetector() {
      var result = false; // if (this.designVersion === PosdConst.FACEBOOK_DESIGN_VERSION_OLD) {           
      //    if (document.querySelector(this.fbConfig.rightColumnAds.container)) {
      //       result = true; 
      //    }
      // } else {           
    ...
    value: function ActivateMutationObserver() {
      ...
      // if (this.designVersion === PosdConst.FACEBOOK_DESIGN_VERSION_OLD) {            
      //     this.fbRightColumn = document.querySelector(this.fbConfig.rightColumnAds.container);
      //  } else {            
    ...
    key: "Detector",
    value: function Detector() {
      var cards = document.querySelectorAll(this.fbConfig.rightColumnAdsNew.cards); // if (this.designVersion === PosdConst.FACEBOOK_DESIGN_VERSION_NEW) {
      //    cards = document.querySelectorAll(this.fbConfig.rightColumnAdsNew.cards);           
      // } else {
      //    cards = document.querySelectorAll(this.fbConfig.rightColumnAds.cards);           
      // }
    ...
    key: "CheckIfAd",
    value: function CheckIfAd(candidate) {
      var isAd = false; // if (this.designVersion === PosdConst.FACEBOOK_DESIGN_VERSION_OLD) {
      //     if (candidate && document.querySelectorAll(this.fbConfig.rightColumnAds.ifAd).length) {
      //        isAd = true;
      //     }
      // } else {

      if (candidate && candidate.querySelectorAll(this.fbConfig.rightColumnAdsNew.ifAd).length) {
        isAd = true;
      } // }
      ...
      script.innerHTML = PosdFacebookVideoDetector.SendVideoData.toString() + "(" + function (id, config) {
            ...
            var _loop = function _loop() {
              ...
              if (needParse) {
                if (parserType !== PARSER_TYPE.INSTREAM_CONFIG) {
                  ...
                  return "break";
                } else if (parserType === PARSER_TYPE.INSTREAM_CONFIG && config.PARSER_CONFIG.INSTREAM_CONFIG.FILTER_DATA) {
                  setTimeout(function () {
        ...
        var parseInStream = function parseInStream(parser, data, url) {
          ...
          try {
            ...
            setTimeout(asyncSendVideoData(_videoData, 'FACEBOOK_INSTREAM_DATA'), 0);
          } catch (err) {
            parsedData = "".concat(data).replace(/\}(\r\n| )\{/g, '}BIS_SEPARATOR{').split(/BIS\_SEPARATOR/g);
            parsedData = parsedData.map(function (el, i) {
              if (el.length > 3) {
          ...
          for (var i = 0; i < parsedData.length && !objectIsReady; i++) {
            var element = parsedData[i];
            formatedObjects[i] = element;
            var _iteratorNormalCompletion2 = true;
            var _didIteratorError2 = false;
            var _iteratorError2 = undefined;

            try {
              for (var _iterator2 = parser.PATH[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                var prop = _step2.value;

                if (formatedObjects[i][prop]) {
                  formatedObjects[i] = formatedObjects[i][prop];
                } else {
                  formatedObjects[i] = null;
                  break;
                }
              }
            } catch (err) {
          ...
          formatedObjects = formatedObjects.filter(function (el) {
            return el;
          });

          if (!formatedObjects.length) {
            return;
          }
          ...
          try {
            for (var _iterator3 = formatedObjects[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
              var el = _step3.value;
              ...
              var index = formatedObjects.indexOf(el);
              var keys = Object.keys(el);
              videoData.content[index] = {};

              for (var _i = 0, _keys = keys; _i < _keys.length; _i++) {
                var key = _keys[_i];

                if (!parser.TICKET_PROPERTIES.includes(key)) {
                  continue;
                }

                var _element = el[key].length ? el[key][0] : el[key];

                videoData.content[index][key] = _element;
              }
            }
          } catch (err) {
            ...
            try {
              for (var _iterator4 = mutationsList[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
                ...
                try {
                  ...
                  for (var _iterator5 = mutation.removedNodes[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
                    var _ret2 = _loop2();
        ...
        if (config.PARSER_CONFIG.SCRIPT.USE) {
          var loadTextDataFromScripts = function loadTextDataFromScripts() {
            ...
            try {
              for (var _iterator6 = scripts[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
                var _script = _step6.value;
            ...
            return function () {
              ...
              try {
                var _loop3 = function _loop3() {
                  var text = _step7.value;
                  setTimeout(function () {
                    return findParserAndParse(text, PARSER_TYPE.STREAM);
                  }, 0);
                };

                for (var _iterator7 = texts[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
                  _loop3();
                }
              } catch (err) {
        ...
        if (config.PARSER_CONFIG.XML_HTTP_REQUEST.USE || config.PARSER_CONFIG.INSTREAM_CONFIG.USE) {
          var open = XMLHttpRequest.prototype.open;
          ...
          XMLHttpRequest.prototype.send = function () {
            ...
            this.onreadystatechange = function () {
              ...
              var matchUrls = config.PARSER_CONFIG.XML_HTTP_REQUEST.MATCH_URLS.concat(config.PARSER_CONFIG.INSTREAM_CONFIG.MATCH_URLS);
              var inStreamHeader = config.PARSER_CONFIG.INSTREAM_CONFIG.HEADERS;
              var requestHeaders = this.requestHeaders ? Object.keys(this.requestHeaders) : null;
              var isInstreamRequest = false;

              if (requestHeaders) {
                isInstreamRequest = requestHeaders.some(function (el) {
                  if (inStreamHeader[el] && _this2.requestHeaders[el] === inStreamHeader[el]) {
                    return true;
                  }
                });
              }
              ...
              if (this.readyState === 4 && matchUrls.filter(function (url) {
                ...
                if (isInstreamRequest && config.PARSER_CONFIG.INSTREAM_CONFIG.IS_ENABLE) {
                  setTimeout(function () {
  ...
  _createClass(PosdFacebookWatch, [{
    ...
    value: function Detector() {
      if (this.designVersion) {
        ...
        if (fbWatch) {
          var posts = Array.from(fbWatch.children);
  ...
  _createClass(PosdRedditAds, [{
    ...
    value: function ActivateDetector(activator) {
      ...
      if (!this.isActive) {
        ...
        var callbackProcessInterval = this.processRedditAds.bind(this, PosdConst.AD_DETECTOR_INITIATOR_TIMER);
        this.processRedditAdsInterval = setInterval(callbackProcessInterval, 1000);

        if (this.AdBlockerAgent.IsEnabledForReddit()) {
          this.processRedditAds(PosdConst.AD_DETECTOR_INITIATOR_ACTIVATION);
          setTimeout(this.processRedditAds.bind(this, PosdConst.AD_DETECTOR_INITIATOR_ACTIVATION), 350);
          setTimeout(this.processRedditAds.bind(this, PosdConst.AD_DETECTOR_INITIATOR_ACTIVATION), 700);
        }

        var callbackInterval = function () {
          _this2.observerNodesChecksAmount = _this2.observerNodesChecksAmount + 1;
          _this2.feedContainer = _this2.getFeedContainer;

          if (!_this2.feedObserver && _this2.feedContainer) {
            var callbackMutationFeed = _this2.processRedditFeedAds.bind(_this2, PosdConst.AD_DETECTOR_INITIATOR_MUTATION);

            _this2.feedObserver = new MutationObserver(callbackMutationFeed);
          ...
          _this2.rightColumnContainer = _this2.getRightColumnContainer;
    ...
    value: function processRedditAds(initiator) {
      this.processRightColumnAds(initiator);
      this.processFeedAds(initiator);
    }
  }, {
    key: "processRedditFeedAds",
    value: function processRedditFeedAds(initiator) {
      this.processFeedAds(initiator);
    }
  }, {
    key: "processFeedAds",
    value: function processFeedAds(initiator) {
      var currentTime = PosdCommon.GetCurrentTimestampMs();
    ...
    value: function processRightColumnAds(initiator) {
      var currentTime = PosdCommon.GetCurrentTimestampMs();
    ...
    value: function buildBannerHtml(cloneCell, type) {
      var contaier = type === PosdConst.REDDIT_AD_PLACEMENT_TYPE_RIGHT_COLUMN ? this.rightColumnContainer : this.feedContainer;
      this.setWrapperClass(contaier);
      var head = this.headCopyWithStyles;
      var html = document.createElement('html'),
          body = this.bodyCopyWithStyles,
          wrapper = contaier.cloneNode();
    ...
    value: function setWrapperStyles(wrapper) {
      wrapper.style.cssText = wrapper.style.cssText + "max-width: ".concat(this.maxWidthFeed, "px");
    }
  }, {
    ...
    value: function isPromoted(cell) {
      ...
      if (cell) {
        var promotedCandidateFeed = Array.from(cell.querySelectorAll(this.redditConfig.FEED_CONTAINER.PROMOTED_SELECTOR)),
            ...
            concatCandidates = promotedCandidateFeed.concat(promotedCandidateRightColumn);
    ...
    value: function processAndSendRedditAd(bisId, adReddit) {
      ...
      if (ad) {
        ...
        if (PosdConfig.CONFIG_MARK_AD_CANDIDATES && PosdConfig.CONFIG_MARK_PROCESSED_AD_CANDIDATES) {
          if (checkOnVideoTag) {
            ad.style.cssText += 'outline: rgba(42, 213, 255, 1) solid 6px !important;';
          } else if (checkOnTextAd) {
            ad.style.cssText += 'outline: rgba(243, 143, 20, 1) solid 6px !important;';
          } else {
            ad.style.cssText += 'outline: green solid 6px !important;';
          }
        }
      }
    }
    ...
    } // listenerMessageCallback(message) {
    //   const { data } = message;
    //   if (data.type === PosdConst.MESSAGE_TYPE_REDDIT_RIGHT_COLUMN_DATA) {
    //     console.log('rightcolumn target url message', data);
    //     window.removeEventListener('message', this.listenerMessageCallback);
    //   }
    // }
    ...
    get: function get() {
      ...
      for (var _i = 0, _candidates = candidates; _i < _candidates.length; _i++) {
        ...
        if (parseInt(elementStyles.height) > minHeight && widthCheck > 2 && parseInt(elementCoordinates.top) > topDistance && candidate.querySelector(exclusionQuery) === null && candidate.nextSibling) {
          result = candidate;
          break;
        }
      }
    ...
    get: function get() {
      ...
      this.redditConfig.NECESSARY_STYLES.BY_DATA_ATTRIBUTE.forEach(function (attributeData) {
        ...
        var _loop = function _loop() {
          var el = _Array$from[_i2];

          if (el.nodeName.search(/^style$/gi) === -1) {
            el.remove();
            return "continue";
          }
          ...
          if (el.dataset[key]) {
            var check = dataValues.some(function (str) {
              return el.dataset.href.includes(str);
            });
        ...
        for (var _i2 = 0, _Array$from = Array.from(head.children); _i2 < _Array$from.length; _i2++) {
          var _ret = _loop();
    ...
    key: "maxWidthFeed",
    get: function get() {
      return this.feedContainer ? "max-width: ".concat(this.feedContainer.offsetWidth, "px;") : '';
    }
  }, {
    ...
    value: function funcToInject() {
      return function (id, config) {
        ...
        function PreSendRightColumnData(data) {
          var fullData = JSON.parse(data.content);
          var result = [];
          var _iteratorNormalCompletion = true;
          var _didIteratorError = false;
          var _iteratorError = undefined;
    ...
    value: function funcToInject() {
      return function (id, config) {
                  ...
                  for (var key in row.data) {
                    ...
                    if (rule_1.select.length > 0) {
                      ...
                      rule_1.select.forEach(function (el) {
                        if (data_1.hasOwnProperty(el.prop)) {
                          var column = el.alias ? el.alias : el.prop;
                          parent.data[column] = row.data[key][el.prop];
                        }
                      });
                      parents.push(parent);
        ...
        var pipes = {
          ...
          jsonQuery: function jsonQuery(data, args) {
            ...
            var arrResults = args.queries.map(function (q, i) {
              return JsonQuery.query(q, independObj[i]).map(function (el) {
                if (!el.hasOwnProperty(args.prop)) {
                  el[args.prop] = [JSON.stringify(el.media[0][0])];
                  delete el.media;
                }
          ...
          filterContains: function filterContains(data, args) {
            var result = data.filter(function (v) {
              if (!v.hasOwnProperty(args.prop)) {
                return false;
              }

              var nextFilter = args.types.some(function (type) {
                return v[args.prop].includes("\"".concat(type, "\""));
              });
              return nextFilter;
            });
            return result;
        ...
        function runPipeLine(parser, data) {
          ...
          try {
            for (var _iterator = parser.pipes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        XMLHttpRequest.prototype.send = function () {
          ...
          var self = this;
          var needUrl = false;
          config.parsers.forEach(function (el) {
            needUrl = _this2.url.includes(el.watch);
          });
