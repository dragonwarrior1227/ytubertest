...
var PosdTicket = /*#__PURE__*/function () {
  ...
  _createClass(PosdTicket, [{
        ...
        if (adContent && adContent.length) {
          ...
          for (var i = 0; i < candidates.length && !isMatched; i++) {
            ...
            if (candidate.length) {
              ...
              try {
                for (var _iterator2 = candidate[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                  var el = _step2.value;
    ...
    value: function TicketBuildingFinished(isForced) {
      ...
      //   let fbVideoData = this.ContentStorage.GetCandidatesFacebookVideoData(this.tabId);          
      //   let newDesign = 0;
      //   let foundVideoDataIds = [];
      //   for (let i = 0; i < this.rawTicket.adsFacebook.length; i++) {
      //     const raw = this.rawTicket.adsFacebook[i];
      ...
      //         let findVideoData = false;
      //         let content = PosdBase64.decode(raw.content);
      //         console.log(`posd_log: ${new Date().toLocaleString()} DEBUG [tabId_${this.tabId}] : (facebook video data), looking for video data`);
      //         for (let j = 0; (j < fbVideoData.length) && !findVideoData; j++) {
      ...
      //             raw.videoData.videoUrl = fbVideoData[j].videoUrl;
      //             raw.videoData.audioUrl = fbVideoData[j].audioUrl;
      //             foundVideoDataIds.push(fbVideoData[j].videoId);
      //           }
      //         }
      //         if (!findVideoData) {
      //           console.log(`posd_log: ${new Date().toLocaleString()} DEBUG [tabId_${this.tabId}] : (facebook video data), cannot find video data!`);
      //         }
      //       } else {
      //         this.lookingForFBVideoData(raw);
      //       }
      //       console.log(`posd_log: ${new Date().toLocaleString()} DEBUG [tabId_${this.tabId}] : (facebook video data), found video data: `, raw);
      //     }
      //   }
      ...
      this.LookingForFacebookAds();
      this.LookingForTwitterAds();
      this.LookingForRedditAds(); // this.rawTicket.adsTwitter = this.ContentStorage.GetCandidatesDataTwitter(this.tabId);
      // let twCandidatesVideoData = this.ContentStorage.GetCandidatesTwitterVideoData(this.tabId);
      // let foundTwitterVideoDataIds = [];
      // if (twCandidatesVideoData && twCandidatesVideoData.length && this.rawTicket.adsTwitter && this.rawTicket.adsTwitter.length) {
      //   for (let ad of this.rawTicket.adsTwitter) {
      //     if (ad.adPlacementType === PosdConst.TWITTER_AD_PLACEMENT_TYPE_FEED && ad.hasOwnProperty('videoData')) {
      //        let adVideoData = this.GetVideoDataForTwitterVideoAd(twCandidatesVideoData, PosdBase64.decode(ad.content));
      ...
      if (this.rawTicket.adCandidatesBanner.length && PosdConfig.CONFIG_MARK_AD_CANDIDATES && PosdConfig.CONFIG_MARK_PROCESSED_AD_CANDIDATES) {
        ...
        _mes.type = PosdConst.MESSAGE_TYPE_BANNER_CANDIDATES_EXTRACTION_STATUSES;
        _mes.to = 'mp_' + this.tabId;
        _mes.content = bannersExtractionStatuses;
      ...
      if (!isForced) {
        if (chainsExtractionStatuses.length && PosdConfig.CONFIG_MARK_AD_CANDIDATES && PosdConfig.CONFIG_MARK_PROCESSED_AD_CANDIDATES) {
          var _mes2 = PosdMessage.EmptyMessage;
          _mes2.type = PosdConst.MESSAGE_TYPE_HTML5_CANDIDATES_EXTRACTION_STATUSES;
          _mes2.to = 'mp_' + this.tabId;
          _mes2.content = chainsExtractionStatuses;
          chrome.tabs.sendMessage(this.tabId, _mes2);
        }
    ...
    value: function LookingForTwitterAds() {
      ...
      if (twCandidatesVideoData && twCandidatesVideoData.length && this.rawTicket.adsTwitter && this.rawTicket.adsTwitter.length) {
        ...
        try {
          for (var _iterator3 = this.rawTicket.adsTwitter[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
            var ad = _step3.value;
    ...
    value: function LookingForRedditAds() {
      ...
      if (this.rawTicket.adsReddit && this.rawTicket.adsReddit.length) {
        ...
        try {
          for (var _iterator4 = this.rawTicket.adsReddit[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
            var ad = _step4.value;
    ...
    value: function LookingForFacebookAds() {
      this.rawTicket.adsFacebook = this.ContentStorage.GetCandidatesDataFacebook(this.tabId);
      ...
      if (this.rawTicket.adsFacebook.length) {
        ...
        for (var i = 0; i < this.rawTicket.adsFacebook.length; i++) {
          var raw = this.rawTicket.adsFacebook[i];
          ...
          if (raw.videoData) {
            if (fbVideoData && fbVideoData.length) {
              ...
              var content = PosdBase64.decode(raw.content);
              ...
              if (!findVideoData) {
                findVideoData = this.buildFBVideoUrl(raw, content);
              }
            }
          }
        }
    ...
    key: "buildFBVideoUrl",
    value: function buildFBVideoUrl(raw, content) {
      var config = this.ConfigManager.GetFacebookConfig();
      var regName = new RegExp(config.videoUrlByPattern.REGEX_ADVERTISER_NAME);
      var regVideoId = new RegExp(config.videoUrlByPattern.REGEX_VIDEO_ID);
      var videoUrl = config.videoUrlByPattern.PATTERN;
      var advertiserName = raw.aboutPageLink.match(regName)[1];
      var videoId = this.getVideoId(content, regVideoId);
      ...
      raw.videoData.videoUrl = videoUrl;
